# Auto MPG Regression Pipeline

## Overview

This project builds and evaluates multiple **regression models** to predict a car's fuel efficiency measured in **miles per gallon (MPG)** using technical and categorical vehicle attributes. The implementation focuses on **linear models**, **regularisation techniques**, and **polynomial feature expansion**, all organised using **scikit-learn pipelines** for clarity, reproducibility, and proper preprocessing.
...
The dataset consists of automobiles produced between **1970 and 1982**, containing both numerical and categorical features.
...
| horsepower   | Engine horsepower (missing values handled)               |
| weight       | Vehicle weight (lbs)                                     |
| acceleration | Time to accelerate from 0 to 60 mph                      |
| model year   | Model year (70 = 1970, …, 82 = 1982)                     |
| origin       | Car origin (1 = USA, 2 = Europe, 3 = Japan)              |
| car name     | Car manufacturer and model (used only for preprocessing) |

The **car name** column is simplified during preprocessing and not used directly for prediction.
...
Each model is trained and evaluated using a fully defined preprocessing and modelling pipeline:
...
Polynomial features of degree 2 are applied to numerical inputs:
...
All preprocessing steps are applied **inside pipelines** to avoid data leakage.
...
* **Learning curves** using cross‑validation
...
```
auto_mpg_regression_pipeline.py
README.md
auto-mpg.csv
```
...
Run the script to train all models, evaluate their performance, and visualise learning curves:

```
python auto_mpg_regression_pipeline.py
```
